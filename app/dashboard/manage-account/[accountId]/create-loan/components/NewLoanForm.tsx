...
const NewLoanForm: React.FC<CustomerAccountProps> = ({
  customer,
  loans,
  currentUser,
}) => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    setIsLoading(true);
    const hasUnclearedLoan = loans?.some(
      (loan) => loan?.clearedStatus === false
    );

    if (hasUnclearedLoan) {
      toast.error(
        "Cannot create new loan when there's an existing loan that has not been marked as cleared. Contact admin for further assistance"
      );
      setIsLoading(false);
      return;
    }
    toast("Creating loan, please wait...");
    ...
    const emailData = {
      ...
      message: `Hello, <br/>Loan application for ${customer?.firstname} ${customer?.lastname} has been successfully created and requires your approval for further actions. <br/> <br/> Go to <a target="_blank" href="https://loan-operations.vercel.app/dashboard/manage-account/${customer?.id}/approve-loan">Manage Loans</a> to approve this customer's loan.
